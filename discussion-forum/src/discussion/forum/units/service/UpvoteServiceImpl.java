...
public class UpvoteServiceImpl implements UpvoteService {
	...
	public Upvote addUpvote(Question question, User user) {
		if (question != null && user != null) {
			...
			if (upvote != null) {
				System.out.println("You have already upvoted");//Displays message if user tries to upvote a question twice
				return upvote;
			}
	...
	public Upvote addUpvote(Reply reply, User user) {
		if (reply != null && user != null) {
			...
			if (upvote != null) {
				System.out.println("You have already upvoted");//Displays message if user tries to upvote a reply twice
				return upvote;
			}
	...
	private Upvote addUpvote(User user, Question question, Reply reply) {
		...
		upvote.setCreated();
		upvotes.add(upvote);
        System.out.println("Upvoted successfully !!");//Displays confirmation message when question/reply is upvoted
		return upvote;
	}
}
